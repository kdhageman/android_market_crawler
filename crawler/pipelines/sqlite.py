...
class PostDownloadPackagePipeline(SqlitePipeline):
    ...
    def create_package(self, item):
        ...
        with self.conn:
            self.conn.execute(qry, (pkg_name, identifier, market, ts, ads_status, app_ads_status, icon_status, privacy_policy_status))
...
class PostDownloadPipeline(SqlitePipeline):
    ...
    def create_version(self, pkg_name, identifier, version, market, sha, status):
        ...
        with self.conn:
            self.conn.execute(qry, (pkg_name, identifier, version, market, sha, status))
